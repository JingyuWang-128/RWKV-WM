...
def build_jepa_world_model(conf, obs_type, env, action_dim, act, device, use_ema_target=False):
    """Build JEPA-RWKV World Model for MuJoCo."""
    if "proprio" in obs_type:
        is_proprio = True
        obs_shape = env.observation_space.shape[0]
    elif "visual" in obs_type:
    ...
    use_self_attention = getattr(wm_conf, 'UseSelfAttention', False)
    self_attn_heads = getattr(wm_conf, 'SelfAttnHeads', 4)
    sigreg_weight = getattr(wm_conf, 'SIGRegWeight', 0.09)
    sigreg_knots = getattr(wm_conf, 'SIGRegKnots', 17)
    sigreg_num_proj = getattr(wm_conf, 'SIGRegNumProj', 1024)
    decoder_weight_init = getattr(wm_conf, 'DecoderWeightInit', 1.0)
    decoder_weight_min = getattr(wm_conf, 'DecoderWeightMin', 0.0)
    decoder_decay_steps = getattr(wm_conf, 'DecoderDecaySteps', 50000)
    decoder_decay_type = getattr(wm_conf, 'DecoderDecayType', 'linear')
    ...
    return JEPAWorldModel(
        ...
        decoder_decay_steps=decoder_decay_steps,
        decoder_decay_type=decoder_decay_type,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-model_type", type=str, default="pwm", choices=["pwm", "jepa"],
                        help="World model type: pwm (original) or jepa (JEPA+RWKV)")
