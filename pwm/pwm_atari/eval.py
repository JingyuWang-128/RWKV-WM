import gymnasium
import argparse
# import cv2  # Only needed for visualization (commented out)
import os
import numpy as np
import torch
...
if __name__ == "__main__":
    ...
    if args.model_type == "jepa":
        ...
        world_model = train.build_jepa_world_model(conf, num_action, act, args.device)
        agent = train.build_agent(conf, num_action, act, args.device, feat_dim=world_model.feat_dim)
    else:
    ...
    root_path = os.path.join(SCRIPT_DIR, "ckpt", args.run_name)
    ...
    if args.model_type == "jepa":
        # JEPA-specific param printing
        if world_model.decoder is not None:
            dec_param = sum(p.numel() for p in world_model.decoder.parameters())
            print(f"Name: Decoder; Params: {(dec_param / 1e6):.3f} M")
        pred_param = sum(p.numel() for p in world_model.predictor.parameters())
        print(f"Name: RWKV Predictor; Params: {(pred_param / 1e6):.3f} M")
    else:
        # PWM-specific param printing
        dec_param = sum(p.numel() for p in world_model.decoder.parameters())
    ...
    for step in tqdm(steps):
        ...
        episode_returns = eval_episodes(
            ...
            seed=args.seed,
            image_size=(conf.BasicSettings.ObsShape[0], conf.BasicSettings.ObsShape[1]),
            world_model=world_model,
            agent=agent
        )
        results.append([step, *episode_returns])

    eval_result_dir = os.path.join(SCRIPT_DIR, "eval_result")
    if not os.path.exists(eval_result_dir):
        os.mkdir(eval_result_dir)

    with open(os.path.join(eval_result_dir, f"{args.run_name}.csv"), "w") as fout:
