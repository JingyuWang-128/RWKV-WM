...
if __name__ == "__main__":
    ...
    parser.add_argument("--rwkv_arch", type=str, default=None, choices=["rwkv6", "rwkv7"])
    args, unknown_args = parser.parse_known_args()
    conf = load_config(args.config_path)
    if args.rwkv_arch is not None:
        conf.defrost()
        conf.Models.WorldModel.RWKVArch = args.rwkv_arch
        conf.freeze()
    if unknown_args:
    ...
    if hasattr(world_model.dynamic, "rnn_layer"):
        rnn_param = sum(p.numel() for p in world_model.dynamic.rnn_layer.parameters())
    elif hasattr(world_model.dynamic, "blocks"):
        rnn_param = sum(p.numel() for p in world_model.dynamic.blocks.parameters())
    else:
        rnn_param = 0
    ...
    print(f"Name: RNN/RWKV; Params: {(rnn_param / 1e6):.3f} M")
    ...
    print(steps)
    results = []
    steps = steps[-1:]  # only eval the last checkpoint
    for step in tqdm(steps):
        world_model.load_state_dict(torch.load(f"{root_path}/world_model_{step}.pth"))
        #****************适配旧结构***************#
        # state_dict = torch.load(f"{root_path}/world_model_{step}.pth")
        # # Backward-compat: older checkpoints saved RWKV blocks under dynamic.blocks.*
        # if any(k.startswith("dynamic.blocks.") for k in state_dict) and not any(
        #     k.startswith("dynamic.core.blocks.") for k in state_dict
        # ):
        #     remapped = {}
        #     for key, val in state_dict.items():
        #         if key.startswith("dynamic.blocks."):
        #             remapped["dynamic.core." + key[len("dynamic."):]] = val
        #         else:
        #             remapped[key] = val
        #     state_dict = remapped
        ...
        episode_returns = eval_episodes(
            num_episode=50, # default 50
            env_name=args.env_name,
            num_envs=20, # default 20
